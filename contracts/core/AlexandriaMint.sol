...
contract AlexandriaMint is ERC1155, ERC1155Supply {
    ...
    function mintPage(address recipient) external onlyManager returns (PayoutDetail memory, uint256 bookID) {
        if (data.getPagesInMintQueue() == 0) revert NoPagesInQueue();
        Book memory _mintBook = data.getMintBook();

        if (exists(_mintBook.startPage)) revert NFTMintedWithCurrentID({id: _mintBook.startPage});
        ...
        emit PageMinted(_mintBook.startPage, recipient, uint32(block.timestamp), _mintBook.carURI);

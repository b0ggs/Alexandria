...
contract AlexandriaV1 is ReentrancyGuard {
    ...
    function withdrawFromTreasury(uint256 amount, address payable recipient) external onlyManager {
    ...
    function settle(uint256 bookId, bool isPassed) external onlyManager nonReentrant {
        ...
        address recipient = isPassed ? disputedBook.proposer : disputedBook.disputer;
        _transferTo(recipient, reward, isPassed ? "Settled Pay Proposer" : "Settled Pay Disputer");
        if (isPassed) {
...
/// @param bookURL The URL of the book.
    function reservePages(bytes32 bookHash, uint256 pageCount, string calldata bookURL) external payable nonReentrant {
            uint256 totalBookBond = pageCount * proposeBond;
            oracle.reservePages(bookHash, pageCount, totalBookBond, msg.sender, bookURL); 
    ...
    function mintPage() external payable nonReentrant {
        (PayoutDetail memory payout, uint256 bookId) = mint.mintPage(msg.sender);
        
        address paymentRecipient = !wasDisputed[bookId] || msg.sender == payout.proposer 
                                ? payout.proposer 
                                : address(this);
    ...
    function mintPageDonation(address msgSender) external payable nonReentrant onlyAllo {
        (PayoutDetail memory payout, uint256 bookId) = mint.mintPage(msgSender);
        
        address paymentRecipient = !wasDisputed[bookId] || msgSender == payout.proposer 
                                ? payout.proposer 
                                : address(this);
    ...
    function _handleMintPayment(address _proposer, uint256 _paymentAmount) internal {
        uint256 totalPayout = _paymentAmount + ((_paymentAmount * bonusPercentage) / 100);
    ...
    function _processPayment(uint256 _requiredAmount, string memory paymentType) internal {
    ...
    function _refund(address recipient, uint256 amount, string memory reason) internal {
        ...
        emit PaymentProcessed(address(this), recipient, amount, 0, amount, reason);
    }
    ...
    /// @param paymentType A string describing the type of payment.
    function _transferTo(address recipient, uint256 amount, string memory paymentType) internal {
        (bool success, ) = payable(recipient).call{value: amount}("");
        if (!success) revert TransferFailed();
        emit PaymentProcessed(address(this), recipient, amount, amount, 0, paymentType);
    }

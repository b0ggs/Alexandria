...
import {IAlexandriaPayoutStrategy} from "./IAlexandriaPayoutStrategy.sol";
import {IRegistry} from "../../core/interfaces/IRegistry.sol";

// Internal Libraries
import {BaseStrategy} from "../BaseStrategy.sol";
import {AlexandriaV1} from "./Alexandria/AlexandriaV1.sol";
...
contract AlexandriaStrategy is BaseStrategy, Initializable, ReentrancyGuard {
    ...
    event AllocationTimeSet(uint256 allocationStartTime, uint256 allocationEndTime);
    event RegistrationTimeSet(uint256 registrationStartTime, uint256 registrationEndTime);
    event StrategyPoolFunded(uint256 amount);
    event PayoutStrategyUpdated(address indexed previousStrategy, address indexed newStrategy);
    ...
    modifier onlyActiveRegistration() {
        require(block.timestamp >= registrationStartTime && block.timestamp <= registrationEndTime, "Registration not active");
        _;
    }
    ...
    constructor(address _allo, string memory _name, address _initialStrategy, address _alexandria) BaseStrategy(_allo, _name) {
    ...
    function initialize(uint256 /*_poolId*/, bytes memory /*_data*/) external pure override {
    ...
    function AlexandriaInitialize(
        ...
        _initializeOrReset(_poolId, _allocateToRecipient, _registrationStartTime, _registrationEndTime, _allocationStartTime, _allocationEndTime);
        emit AlexandriaInitialized(_poolId, _allocateToRecipient, _registrationStartTime, _registrationEndTime, _allocationStartTime, _allocationEndTime);
    }
    ...
    function resetStrategy(
        ...
        _initializeOrReset(_poolId, _allocateToRecipient, _registrationStartTime, _registrationEndTime, _allocationStartTime, _allocationEndTime);
        emit StrategyReset(_poolId, _allocateToRecipient, _registrationStartTime, _registrationEndTime, _allocationStartTime, _allocationEndTime);
    }
    ...
    /// @param _allocationStartTime The start time for allocation.
    /// @param _allocationEndTime The end time for allocation.
    function setAllocationTime(uint256 _allocationStartTime, uint256 _allocationEndTime) external onlyOwner{
    ...
    /// @param _registrationStartTime The start time for registration.
    /// @param _registrationEndTime The end time for registration.
    function setRegistrationTime(uint256 _registrationStartTime, uint256 _registrationEndTime) external onlyOwner {
        require(block.timestamp < _registrationStartTime && _registrationStartTime < _registrationEndTime, "Invalid timestamps");
    ...
    function fundStrategyPool(uint256 _amount) external payable onlyOwner nonReentrant {
    ...
    function getRecipientStatus(address _recipientAddress) external view override returns (RecipientStatus) {
    ...
    function registerRecipient(address recipient) external onlyActiveRegistration {
        require(_registry.isOwnerOrMemberOfProfile(keccak256(abi.encode(recipient)), msg.sender), "Not a registered profile in IRegistry");
        require(!registeredRecipients[recipient], "Recipient already registered");
    ...
    function donateAndmint(address project) external payable nonReentrant onlyWhenPoolActive {
        require(registeredRecipients[project], "Project is not a registered recipient");
        uint256 mintCost = payoutStrategy.mintCost(); 
        uint256 minimumDonation = payoutStrategy.minimumDonation();
        require(msg.value >= mintCost + minimumDonation, "Insufficient funds sent");
    ...
    function withdraw(uint256 _amount) external onlyPoolManager(msg.sender) onlyWhenPoolActive {
       ...
       require(_amount <= poolAmount, "Amount exceeds available pool funds");
        ...
        _transferAmount(alloContract.getPool(poolId).token, msg.sender, _amount);
    ...
    function distribute() external onlyOwner onlyWhenPoolActive {
        require(block.timestamp > allocationEndTime, "Allocation period is not over yet");
        ...
        if (allocateToRecipient) {
            amounts = payoutStrategy.calculateDistributions(recipients, poolAmount - totalDonations);
            for (uint256 i = 0; i < recipients.length; i++) {
                uint256 totalAmountForRecipient = amounts[i] + recipientDonations[recipients[i]];
                _transferAmount(poolToken, recipients[i], totalAmountForRecipient);
                totalDistributed += amounts[i];
                recipientDonations[recipients[i]] = 0;
                emit FundsDistributed(recipients[i], totalAmountForRecipient);
            }
        } else {
            amounts = payoutStrategy.calculateDistributions(recipients, poolAmount);
            for (uint256 i = 0; i < recipients.length; i++) {
    ...
    function _initializeOrReset(
        ...
        require(block.timestamp < _registrationStartTime && _registrationStartTime < _registrationEndTime, "Invalid registration timestamps");
        registrationStartTime = _registrationStartTime;
        registrationEndTime = _registrationEndTime;

        require(block.timestamp < _allocationStartTime && _allocationStartTime < _allocationEndTime, "Invalid allocation timestamps");
    ...
    function _setAllocationTime(uint256 _allocationStartTime, uint256 _allocationEndTime) internal {
        require(block.timestamp < _allocationStartTime && _allocationStartTime < _allocationEndTime, "Invalid timestamps");
        allocationStartTime = _allocationStartTime;
        allocationEndTime = _allocationEndTime;
    }
    ...
    function _getRecipientStatus(address _recipientAddress) internal view override returns (RecipientStatus) {
        if (!_registry.isOwnerOrMemberOfProfile(keccak256(abi.encode(_recipientAddress)), _recipientAddress)) {
    ...
    function _allocate(bytes memory _data, address /*_sender*/) internal override {
        (address decodedRecipient, uint256 decodedAmount) = abi.decode(_data, (address, uint256));

        require(decodedRecipient != address(0), "Invalid recipient address");
        require(decodedAmount > 0 && decodedAmount <= poolAmount, "Invalid allocation amount");
        require(registeredRecipients[decodedRecipient], "Recipient not registered or not in 'Accepted' state");
    ...
    function _getPayout(address _recipientId, bytes memory) internal view override returns (PayoutSummary memory) {
        address[] memory recipients = _getAllRecipients();
        uint256 payoutAmount = payoutStrategy.getPayoutAmount(_recipientId, poolAmount, recipients);
